...
/***/ function(module, exports, __webpack_require__) {
	...
	*/
	...
	function Height() {
		...
		(0, _jquery2.default)('#main').css('height', iHeight);
		var $len = (0, _jquery2.default)('.start').length;
		(0, _jquery2.default)('.start').css({
...
/***/ function(module, exports, __webpack_require__) {
	...
	function navEvent() {
		...
		(0, _jquery2.default)('.nav').delegate('li', 'click', function () {
			...
			var $num = parseInt((0, _jquery2.default)(this).index()) * 82 + 6;
			t.to('.line', 0.5, { left: $num }, 0);
			var $rotate = parseInt((0, _jquery2.default)(this).index()) * 90 + 360;

			t.to('.item', 1, { rotation: $rotate }, 0);

			var $text = (0, _jquery2.default)(this).find('a').text();
			var $conTop = parseInt((0, _jquery2.default)('.contentImg').css('top'));
			var $nTop = parseInt((0, _jquery2.default)('.note').css('top'));
			...
			(0, _jquery2.default)(this).css('background', '#4a8e2f');
			(0, _jquery2.default)(this).find('a').css('color', '#32629b');

			var $rotate = parseInt((0, _jquery2.default)(this).index()) * 90 + 360 + 90;
			t.to('.item', 0.3, { rotation: $rotate }, 0);
		}).delegate('li', 'mouseleave', function () {
			var t = new TimelineMax();

			(0, _jquery2.default)(this).css('background', '');
			(0, _jquery2.default)(this).find('a').css('color', '#000');
	...
	function workStyle() {
		(0, _jquery2.default)('.t-work').css({
			width: 0
		});
		(0, _jquery2.default)('.circle').css({
			opacity: 0,
			width: 0,
			height: 0
		});
		(0, _jquery2.default)('.v-line').css({
			opacity: 0,
			height: 0
		});
...
/***/ function(module, exports, __webpack_require__) {
	...
	function circleFirst(index, num, time) {
		...
		setTimeout(function () {
			(0, _jquery2.default)('.v-content').eq(index).animate({ opacity: 1 }, 400);
		}, time3);
	}
	...
	function createLi(obj) {
		...
		(0, _jquery2.default)('.list').append((0, _jquery2.default)(str));
	}

	function createTitle(obj) {
		var str = '<p _id=' + obj.id + '>' + obj.num + '、' + obj.title + '</p>';
		(0, _jquery2.default)('.title').append((0, _jquery2.default)(str));
	}

	function createContentBox(test) {
		var str = '<div class="content-box">' + test + '</div>';
		(0, _jquery2.default)('.content').append((0, _jquery2.default)(str));
	}
	...
	function workEvent() {
		...
		(0, _jquery2.default)('.t-work').delegate('.v-content', 'click', function () {
			...
			for (var i = 0; i < _workData.workData.length; i++) {
				createLi(_workData.workData[i]);
			}
			for (var _i = 0; _i < _workData.workTitleData[$_title].length; _i++) {
				createTitle(_workData.workTitleData[$_title][_i]);
			}
		...
		(0, _jquery2.default)('.list').delegate('li', 'click', function () {
			...
			(0, _jquery2.default)('.content').css('display', 'none');
			var $_title = (0, _jquery2.default)(this).attr('_title');
			for (var i = 0; i < _workData.workTitleData[$_title].length; i++) {
				createTitle(_workData.workTitleData[$_title][i]);
			}
		});
		(0, _jquery2.default)('.title').delegate('p', 'click', function () {
			var $_id = (0, _jquery2.default)(this).attr('_id');
			(0, _jquery2.default)('.content').html('');
			createContentBox(_workData.workContentData[$_id]);
			(0, _jquery2.default)('.title').css('display', 'none');
...
/***/ function(module, exports, __webpack_require__) {
	...
	function note() {
	    ...
	    var data = _photoData.photoData;

	    var $heightNum = parseInt((0, _jquery2.default)('.wrap').height()) / 160;
	    var $widthNum = parseInt((0, _jquery2.default)('.wrap').width()) / 210;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _profileData = __webpack_require__(11);
	...
	function profileInit() {
		...
		for (var i = 0; i < _profileData.data.length; i++) {
			createArticle(_profileData.data[i].item, _profileData.data[i].content, _profileData.data[i].time);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _footerData = __webpack_require__(13);
...
/***/ function(module, exports, __webpack_require__) {
	...
	}else if(this._time<0){this._time=time=0;}else {time=this._time;}}}}if(this._cycle!==prevCycle)if(!this._locked){ /*
	...
	return;}this._time=recTime;this._totalTime=recTotalTime;this._cycle=recCycle;this._rawPrevTime=recRawPrevTime;}if((this._time===prevTime||!this._first)&&!force&&!internalForce){if(prevTotalTime!==this._totalTime)if(this._onUpdate)if(!suppressEvents){ //so that onUpdate fires even during the repeatDelay - as long as the totalTime changed, we should trigger onUpdate.
	...
	_specialProps={},p=CSSPlugin.prototype=new TweenPlugin("css");p.constructor=CSSPlugin;CSSPlugin.version="1.17.0";CSSPlugin.API=2;CSSPlugin.defaultTransformPerspective=0;CSSPlugin.defaultSkewType="compensated";CSSPlugin.defaultSmoothOrigin=true;p="px"; //we'll reuse the "p" variable to keep file size down
	...
	_parsePosition=function _parsePosition(v,recObj){if(v==null||v===""||v==="auto"||v==="auto auto"){ //note: Firefox uses "auto auto" as default whereas Chrome uses "auto".
	...
	l=ba.length,autoRound=_autoRound!==false,i,xi,ni,bv,ev,bnums,enums,bn,rgba,temp,cv,str;if(e.indexOf(",")!==-1||b.indexOf(",")!==-1){ba=ba.join(" ").replace(_commasOutsideParenExp,", ").split(" ");ea=ea.join(" ").replace(_commasOutsideParenExp,", ").split(" ");l=ba.length;}if(l!==ea.length){ //DEBUG: _log("mismatched formatting detected on " + p + " (" + b + " vs " + e + ")");
	ba=(dflt||"").split(" ");l=ba.length;}pt.plugin=plugin;pt.setRatio=setRatio;for(i=0;i<l;i++){bv=ba[i];ev=ea[i];bn=parseFloat(bv); //if the value begins with a number (most common). It's fine if it has a suffix like px
	...
	}else if(clrs&&(bv.charAt(0)==="#"||_colorLookup[bv]||_rgbhslExp.test(bv))){str=ev.charAt(ev.length-1)===","?"),":")"; //if there's a comma at the end, retain it.
	bv=_parseColor(bv);ev=_parseColor(ev);rgba=bv.length+ev.length>6;if(rgba&&!_supportsOpacity&&ev[3]===0){ //older versions of IE don't support rgba(), so if the destination alpha is 0, just use "transparent" for the end color
	...
	rgba=false;}pt.appendXtra(rgba?"rgba(":"rgb(",bv[0],ev[0]-bv[0],",",true,true).appendXtra("",bv[1],ev[1]-bv[1],",",true).appendXtra("",bv[2],ev[2]-bv[2],rgba?",":str,true);if(rgba){bv=bv.length<4?1:bv[3];pt.appendXtra("",bv,(ev.length<4?1:ev[3])-bv,str,false);}}}else {bnums=bv.match(_numExp); //gets each group of numbers in the beginning value string and drops them into an array
	...
	cssp._lastParsedTransform=vars;var originalGSTransform=t._gsTransform,m1=cssp._transform=_getTransform(t,_cs,true,vars.parseTransform),style=t.style,min=0.000001,i=_transformProps.length,v=vars,endRotations={},transformOriginString="transformOrigin",m2,skewY,copy,orig,has3D,hasChange,dr,x,y;if(typeof v.transform==="string"&&_transformProp){ //for values like transform:"rotate(60deg) scale(0.5, 0.8)"
	...
	copy.position="absolute";_doc.body.appendChild(_tempDiv);m2=_getTransform(_tempDiv,null,false);_doc.body.removeChild(_tempDiv);if(v.xPercent!=null){m2.xPercent=_parseVal(v.xPercent,m1.xPercent);}if(v.yPercent!=null){m2.yPercent=_parseVal(v.yPercent,m1.yPercent);}}else if((typeof v==="undefined"?"undefined":_typeof(v))==="object"){ //for values like scaleX, scaleY, rotation, x, y, skewX, and skewY or transform:{...} (object)
	...
	bs=bn+"%";}}else if(esfx==="em"){bn/=_convertToPixels(target,p,1,"em"); //otherwise convert to pixels.
	 ...
	 * RoundPropsPlugin
	 * ----------------------------------------------------------------
	 */(function(){var RoundPropsPlugin=_gsScope._gsDefine.plugin({propName:"roundProps",priority:-1,API:2, //called when the tween renders for the first time. This is where initial values should be recorded and any setup routines should run.
	init:function init(target,value,tween){this._tween=tween;return true;}}),p=RoundPropsPlugin.prototype;p._onInitAllProps=function(){var tween=this._tween,rp=tween.vars.roundProps instanceof Array?tween.vars.roundProps:tween.vars.roundProps.split(","),i=rp.length,lookup={},rpt=tween._propLookup.roundProps,prop,pt,next;while(--i>-1){lookup[rp[i]]=1;}i=rp.length;while(--i>-1){prop=rp[i];pt=tween._firstPT;while(pt){next=pt._next; //record here, because it may get removed
	if(pt.pg){pt.t._roundProps(lookup,true);}else if(pt.n===prop){this._add(pt.t,prop,pt.s,pt.c); //remove from linked list
	 ...
	 * AttrPlugin
	 * ----------------------------------------------------------------
	 */(function(){var _numExp=/(?:\d|\-|\+|=|#|\.)*/g,_suffixExp=/[A-Za-z%]/g;_gsScope._gsDefine.plugin({propName:"attr",API:2,version:"0.4.0", //called when the tween renders for the first time. This is where initial values should be recorded and any setup routines should run.
	init:function init(target,value,tween){var p,start,end,suffix,i;if(typeof target.setAttribute!=="function"){return false;}this._target=target;this._proxy={};this._start={}; // we record start and end values exactly as they are in case they're strings (not numbers) - we need to be able to revert to them cleanly.
	this._end={};this._suffix={};for(p in value){this._start[p]=this._proxy[p]=start=target.getAttribute(p)+"";this._end[p]=end=value[p]+"";this._suffix[p]=suffix=_suffixExp.test(end)?end.replace(_numExp,""):_suffixExp.test(start)?start.replace(_numExp,""):"";if(suffix){i=end.indexOf(suffix);if(i!==-1){end=end.substr(0,i);}}if(!this._addTween(this._proxy,p,parseFloat(start),end,p)){this._suffix[p]=""; //not a valid tween - perhaps something like an <img src=""> attribute.
	...
	set:function set(ratio){this._super.setRatio.call(this,ratio);var props=this._overwriteProps,i=props.length,lookup=ratio===1?this._end:ratio?this._proxy:this._start,useSuffix=lookup===this._proxy,p;while(--i>-1){p=props[i];this._target.setAttribute(p,lookup[p]+(useSuffix?this._suffix[p]:""));}}});})(); /*
	 * ----------------------------------------------------------------
	...
	if(true){ //AMD
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [], __WEBPACK_AMD_DEFINE_RESULT__ = function(){return cl;}.apply(exports, __WEBPACK_AMD_DEFINE_ARRAY__), __WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__));}else if(ns===moduleName&&typeof module!=="undefined"&&module.exports){ //node
	...
	if(tl._timeline){while(tl._timeline){if(tl._timeline._time!==(tl._startTime+tl._totalTime)/tl._timeScale){tl.totalTime(tl._totalTime,true);}tl=tl._timeline;}}}if(this._gc){this._enabled(true,false);}if(this._totalTime!==time||this._duration===0){this.render(time,suppressEvents,false);if(_lazyTweens.length){ //in case rendering caused any tweens to lazy-init, we should render them because typically when someone calls seek() or time() or progress(), they expect an immediate render.
